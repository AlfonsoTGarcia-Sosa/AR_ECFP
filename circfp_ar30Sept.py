...
dc.utils.download_url("https://hermes.chem.ut.ee/~alfx/ML/train2.csv",current_dir
)
...
g.columns
g.drop(columns=['s_m_title','cas','cid', 'gsid','dsstox_substance_id', 'preferred_name','InChI_Code_QSARr','InChI Key_QSARr', 'BindingClass'])
g.to_csv('balanced_droppedcols2.csv',index=False)
"""Re-upload training set, now balanced, and with less properties"""
current_dir = os.path.dirname(os.path.realpath("__file__"))
dc.utils.download_url("https://hermes.chem.ut.ee/~alfx/ML/balanced_droppedcols2.csv",  current_dir)
dataset_file = "balanced_droppedcols2.csv"

dataset = loader.featurize(dataset_file)
dataset
"""Download evaluation set"""
current_dir = os.path.dirname(os.path.realpath("__file__"))
dc.utils.download_url("https://hermes.chem.ut.ee/~alfx/ML/Eval_4smi.csv",  current_dir)
eval_df = pd.read_csv("Eval_4smi.csv")
eval_df
bal_eval = eval_df
eval_dataset_file = "Eval_4smi.csv"
